...
def test_failed_password():
    input_line = "kvasir sshd[58242]: Failed password for root from 200.73.138.19 port 55624 ssh2"
    patterns = block.build_regex_list(block.PROCESS_CFG['auth_expressions'])
    rv, details = block.check_log_line(input_line.rstrip(), patterns)
    assert rv


def test_invalid_user():
    input_line =\
        "kvasir sshd[58362]: Failed password for invalid user pi from 2.34.166.62 port 34356 ssh2"
    patterns = block.build_regex_list(block.PROCESS_CFG['auth_expressions'])
    rv, details = block.check_log_line(input_line.rstrip(), patterns)
    assert rv
...
def test_failed_none():
    input_line =\
        "kvasir sshd[58318]: Failed none for invalid user admin from 59.25.189.150 port 39177 ssh2"
    patterns = block.build_regex_list(block.PROCESS_CFG['auth_expressions'])
    rv, details = block.check_log_line(input_line.rstrip(), patterns)
    assert rv
...
def test_disconnect_preauth():
    input_line = "kvasir sshd[58584]: Received disconnect from 61.177.173.14 port 31106:11:  [preauth]"
    patterns = block.build_regex_list(block.PROCESS_CFG['auth_expressions'])
    rv, details = block.check_log_line(input_line.rstrip(), patterns)
    assert rv
...
def test_disconnect_byebye():
    input_line = \
        "kvasir sshd[58371]: Received disconnect from 205.214.74.6 port 40020:11: Bye Bye [preauth]"
    patterns = block.build_regex_list(block.PROCESS_CFG['auth_expressions'])
    rv, details = block.check_log_line(input_line.rstrip(), patterns)
    assert rv
...
def test_kex_method_drop():
    input_line = \
        "kvasir sshd[54535]: Unable to negotiate with 141.98.10.210 port 45474: " \
